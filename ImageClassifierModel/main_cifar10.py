...
def main(Batch_Size, Epochs, Learning_Rate, Experiment):
    ...
    dataset = torchvision.datasets.CIFAR10(root='./ImageClassifierModel/data', train=True, download=True, transform=transform)
    ...
    folder_path = f'./ImageClassifierModel/Saved_Models/Experiment_{Experiment}'

    # Check if the folder exists, if not, create it
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    
    torch.save(net.state_dict(), f'./ImageClassifierModel/Saved_Models/Experiment_{Experiment}/{accuracy:.2f}_{Experiment}cifar10_cnn.pth')
